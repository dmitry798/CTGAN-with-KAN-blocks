...
Тут архитектура отчасти поломана, нужно потом еще подумать че с этим делать"""
...
from torch.nn import BatchNorm1d, Dropout, Linear, Module, LeakyReLU, SiLU, Sequential, functional
...
# (!) Новый skip connection для KAN (заменил линейный слой и ФА)
class KAN_Residual(Module):
    ...
    def __init__(self, i, o,
                ...
                grid_range=[-1, 1]):
        super().__init__()
        self.fc = KANLinear(i, o, grid_size=grid_size, 
...
class KAN_CTGAN(BaseSynthesizer):
    ...
    Args:
        ...
        cuda (bool):
            **Deprecated** Whether to attempt to use cuda for GPU computation.
            ...
            Defaults to ``True``.
    """
    # (!) Добавил параметры для KAN, target_epsilon - желаемый уровень приватности, train_delta - вероятность утечки
    def __init__(
        self,
    ...
    def fit(self, train_data, discrete_columns=(), epochs=None):
        ...
        # (!) dummy_loader нужен только чтобы передать размер батча и размер всего датасета в Opacus
        dummy_dataset = TensorDataset(torch.randn(len(train_data), 1))
        dummy_loader = DataLoader(dummy_dataset, batch_size=self._batch_size)
        ...
        for i in epoch_iterator:
            for id_ in range(steps_per_epoch):
                for n in range(self._discriminator_steps):
                    ...
                    loss_real = functional.binary_cross_entropy_with_logits(y_real, torch.ones_like(y_real))
                ...
                if hasattr(discriminator, 'no_grad_samples'):
                    with discriminator.no_grad_samples():
                        loss_g.backward()
                else:
                    loss_g.backward()
                optimizerG.step()
